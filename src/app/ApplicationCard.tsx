...
const ApplicationCard = () => {
  const [versionPickedToDownload, setVersionPickedToDownload] =
    useState<number>(0);
  ...
  const updatePickedVersion = (version: string) => {
    setVersionPickedToDownload(
      versions.findIndex((itemVersion) => itemVersion === version)
    );
  };

  return (
    <div className="flex flex-col gap-2 shadow-2xl rounded-2xl px-6 py-5 border-black/10 border max-w-sm">
      <div className="text-xs">License Type: 5 License - Yearly</div>
      <LogoIcon />
      <div className="flex flex-col gap-4">
        <div className="">
          <div className="text-xs">- Project Name</div>
          <div className="text-2xl font-black">LogoIpsum App</div>
        </div>

        <div className="flex flex-col gap-4 rounded-lg px-4 py-3 bg-gray-200">
          <div className="text-sm text-gray-900 font-black">
            Pick a Version to download
          </div>
          <div className=" text-xs">Latest Version: v3.8.0</div>

          <div className="flex flex-wrap gap-2">
            {versions.map((version, idx) => {
              if (versions[versionPickedToDownload] === version) {
                return (
                  <div
                    key={idx}
                    className="bg-black flex items-center gap-2 text-white rounded-md w-max px-3 py-1"
                  >
                    <CheckCircledIcon className="h-6 w-6" /> {version}
                  </div>
                );
              } else {
                return (
                  <button
                    key={idx}
                    onClick={() => updatePickedVersion(version)}
                    className="bg-white rounded-md w-max px-3 py-1"
                  >
                    {version}
                  </button>
                );
      ...
      <button className="bg-black text-white text-2xl font-black w-full flex justify-center items-center py-3 rounded-md">
        Download {versions[versionPickedToDownload]}
      </button>
    </div>
  );
};
